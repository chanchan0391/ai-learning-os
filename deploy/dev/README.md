...
应用监控在读取 release 或发起网络探测前还会用只读系统镜像中的绝对 `stat` 引导工具验证：`systemctl`、`curl` 与 Node 必须解析为绝对路径，只能由 root 或当前用户拥有，且其文件与直接父目录不可被组或其他用户写入；符号链接和额外硬链接同样被拒绝。这样用户管理器中意外污染的 `PATH` 不能用替代工具伪造服务、网络或 JSON 健康证明。
