...
7. 只保留最近三个 release，并清理超过一天的完整或未完成上传归档及 `.deploy-*` 临时工作区；当前 revision 的托管归档在校验或部署失败时也会删除，避免反复失败或不可捕获终止持续占用服务器磁盘。清理在部署锁内执行，只匹配受管目录的直属制品，并保留近期文件供安全重试和故障调查。publisher 使用 macOS 自带的 `shlock` 原子记录进程归属；进程异常退出后，下一轮会识别失效 PID 并回收锁，不会永久停止自动发布。SSH/SCP 连接使用连接超时和 keepalive 失联判定；release 下载同时具有总时间和低速中止边界，网络停滞会让本轮明确失败并由后续定时轮次重试。
