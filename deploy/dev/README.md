...
远端每日运行 `backup.sh`，以 PostgreSQL custom format 保存数据库备份；备份目录必须使用绝对路径，并在任何权限变更或数据库访问前证明它是真实目录且归当前用户所有，拒绝符号链接、普通文件和跨用户目录。脚本使用 `flock` 串行化定时任务与发布前备份，真实并发会在访问 PostgreSQL 前失败，进程异常退出后内核会自动释放锁，遗留的私有锁文件不会阻塞下一次执行。脚本会先拒绝空输出，再用 `pg_restore --list` 验证归档可读取，只有通过验证的临时文件才会以碰撞安全的名称发布。每份归档同时生成只引用文件名的 SHA-256 sidecar；备份目录权限为 `0700`，锁文件、归档与校验文件权限为 `0600`。每轮只清理目录直属的受管文件：超过 7 天的归档、配对或孤立校验文件，以及超过一天的异常终止临时文件；不会递归删除嵌套路径。该开发基线不代替生产环境的异地加密备份。
