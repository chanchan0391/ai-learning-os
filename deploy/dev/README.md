...
6. 健康后用已验证 release 中的版本原子更新远程 `deploy-main.sh`、`backup.sh`、`backup-health.sh`、只读 `verify-backup.sh`、隔离 `restore-drill.sh` 与共享 Docker 客户端解析助手；publisher 发现同一 revision 时仍会进入远端轻量对账，并只在内容或执行权限漂移时刷新它们，避免激活后中断让后续部署、每日备份、健康监控或恢复演练继续使用旧逻辑。
