...
API 与 Web 服务的 `ExecStopPost` 只在 systemd 报告非成功结果时调用 `crash-evidence.sh`，在 release 外的私有 `operations-state` 目录原子递增服务级计数。计数不含时间、退出消息、用户或学习内容，显式服务重启、部署和主机重启不会清除。应用监控对每次新增计数输出一次稳定失败事件并推进私有观察游标；当前激活周期内的 `NRestarts` 检查仍保持失败信号直到显式重启。记录器和监控都会拒绝符号链接、硬链接、共享写入或异常回退的计数状态。
