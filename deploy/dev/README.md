...
5. 在有界连接与响应时间内验证两个用户服务真正使用选定的 Node 二进制、Web 首页和 API 健康端点，同时要求 API 报告的 release revision 与待部署提交完全一致，并确认实时模型、同步和 PostgreSQL 就绪检查均通过；周期监控还会在探测前要求部署根、release 根与活动 release 归当前用户独占写入，拒绝带符号链接祖先的非规范路径，并检查更高层祖先不存在无 sticky 保护的共享写入边界，避免其他本地账号替换静态 Web 制品后复用 API revision 证明；失败时恢复上一 release、重启服务，并使用上一提交标识重新通过完整健康门，明确报告回滚是否真正恢复服务。
6. 健康后用已验证 release 中的版本原子更新远程 `deploy-main.sh`、`backup.sh`、`backup-health.sh`、`application-health.sh`、`crash-evidence.sh`、`operations-metrics.sh`、`record-monitor-success.sh`、`host-capacity.sh`、只读 `verify-backup.sh`、隔离 `restore-drill.sh` 与共享 Docker 客户端解析助手；publisher 发现同一 revision 时仍会进入远端轻量对账，并只在内容或执行权限漂移时刷新它们，避免激活后中断让后续部署、每日备份、健康监控或恢复演练继续使用旧逻辑。
7. 只保留最近三个已验证 release；健康门失败时，在回滚或停止服务后删除未通过验证且不再活动的 release。部署还会清理超过一天的完整或未完成上传归档及 `.deploy-*` 临时工作区；当前 revision 的托管归档在校验或部署失败时也会删除，避免连续失败或不可捕获终止持续占用服务器磁盘。所有删除都在部署锁内执行，只匹配受管目录的直属制品，并在删除前重新确认目标是当前用户拥有的真实目录；失败 release 还必须确认不是当前版本。备份 runner 在创建目录或锁前将 `flock` 与 `stat` 解析为绝对路径，验证工具及父目录归 root 或当前用户所有且不可共享写入；当前用户工具必须只有一个硬链接，root 管理的只读系统工具可保留发行版硬链接，裸 PATH 工具进一步要求 root 归属，systemd 只读映射只对精确系统路径开放。远端 runner 从启动起使用私有 umask，在加锁、清理或写入前要求部署根目录与受管子目录均为当前用户拥有的真实目录，并在校验后将这些专用目录收敛为 `0700`；它同时拒绝符号链接、非普通、不归部署用户所有或具有多个硬链接的现有锁目标。release 内 runner 与已安装 runner 同样要求当前用户独占的普通文件，更新通过不可预测临时文件原子完成，避免共享 inode 或预置 staged 路径把内容写向非受管目标。publisher 每轮都会拒绝符号链接形式、不归当前用户所有或允许组/其他用户写入的缓存父目录、checkout 与 `.git`，并验证其 `origin` 与配置仓库完全一致，再从该远端解析不可变 revision，避免缓存重定向、跨用户替换或本地篡改静默改变部署来源；首次 clone 前会完成同样的父目录校验，clone 后重新验证生成目录。新 revision 激活后，publisher 会立即再进入刚刷新好的部署 runner 执行一次同版本对账，因此新版本首次引入的运维脚本也会在本轮验证并安装，不依赖下一轮 publisher。publisher 使用 macOS 自带的 `shlock` 原子记录进程归属；进程异常退出后，下一轮会识别失效 PID 并回收锁，不会永久停止自动发布。publisher 日志达到 5 MiB 后在发布锁内轮转，保留四代近期诊断记录；触发轮转的当前轮次会继续写入第一代文件，下一轮由 launchd 创建新的主日志路径，因此不会截断仍打开的文件描述符。可用 `AI_LEARNING_PUBLISH_LOG_MAX_BYTES` 调整单代阈值。SSH/SCP 连接使用连接超时和 keepalive 失联判定；release 下载同时具有总时间和低速中止边界，网络停滞会让本轮明确失败并由后续定时轮次重试。控制面安装在写入 unit 或备份前要求部署目录、systemd 用户 unit 目录和控制面备份目录均为当前用户拥有的真实目录，并拒绝符号链接锁文件；锁与备份分别收紧为 `0600` 与 `0700`。
...
应用监控在读取 release 或发起网络探测前还会用只读系统镜像中的绝对 `stat` 引导工具验证：`systemctl`、`curl` 与 Node 必须解析为绝对路径，且其文件与直接父目录不可被组或其他用户写入；符号链接和额外硬链接同样被拒绝。通过 `PATH` 解析的裸命令名要求文件与父目录均由 root 拥有，只有显式绝对路径才允许当前部署用户拥有的受管运行时；身份与 revision 读取辅助工具固定为只读系统路径。systemd 只读系统视图将 root 映射为 nobody 时，仅在存在 invocation ID 且目标严格为 `/usr/bin/systemctl` 或 `/usr/bin/curl` 时接受该映射。这样用户管理器中意外污染的 `PATH` 不能用替代工具伪造服务、release、网络或 JSON 健康证明。
...
API 与 Web 服务的 `ExecStopPost` 只在 systemd 报告非成功结果时调用 `crash-evidence.sh`，在 release 外的私有 `operations-state` 目录原子递增服务级计数。计数不含时间、退出消息、用户或学习内容，显式服务重启、部署和主机重启不会清除。记录器与应用监控通过控制面预置的稳定 `flock` 文件串行访问这组计数；锁由部署用户独占且不随原子计数替换改变 inode，快速连续重启不会读取同一个旧值，监控也不会在递增过程中推进错误的观察游标。锁等待最多五秒，超时会保留旧证据并明确失败。应用监控对每次新增计数输出一次稳定失败事件并推进私有观察游标；当前激活周期内的 `NRestarts` 检查仍保持失败信号直到显式重启。记录器、监控和控制面都会拒绝符号链接、硬链接、共享写入或异常回退的计数与锁状态。备份健康、隔离恢复演练和主机容量 service 还会在主检查完整成功后运行固定名 `ExecStartPost` 记录器，分别原子更新私有成功时间；主检查失败不会推进时间，记录失败则使该 service 明确失败。`operations-metrics.sh` 在同一锁的共享模式下输出只读 Prometheus 文本，包括累计崩溃计数、尚未被应用监控观察的崩溃数量，以及四类监控最近成功时间；任一制品不安全时不会输出部分指标：
