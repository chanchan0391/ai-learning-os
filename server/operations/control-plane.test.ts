...
function backupMonitorServiceContents() {
  return `[Service]\nType=oneshot\nExecStart=%h/services/ai-learning-os/backup-health.sh\nExecStartPost=%h/services/ai-learning-os/current/deploy/dev/record-monitor-success.sh backup-monitor\nReadWritePaths=%h/services/ai-learning-os/operations-state\n${monitorSandboxDirectives.join("\n")}\n`;
}
...
function restoreDrillServiceContents() {
  return `[Unit]\nAfter=ai-learning-os-backup.service\n[Service]\nType=oneshot\nExecStart=%h/services/ai-learning-os/restore-drill.sh\nExecStartPost=%h/services/ai-learning-os/current/deploy/dev/record-monitor-success.sh restore-drill\nReadWritePaths=%h/services/ai-learning-os/operations-state\nTimeoutStartSec=15m\n${monitorSandboxDirectives.join("\n")}\n`;
}
...
function capacityMonitorServiceContents() {
  return `[Service]\nType=oneshot\nExecStart=%h/services/ai-learning-os/host-capacity.sh\nExecStartPost=%h/services/ai-learning-os/current/deploy/dev/record-monitor-success.sh host-capacity-monitor\nReadWritePaths=%h/services/ai-learning-os/operations-state\n${monitorSandboxDirectives.join("\n")}\n`;
}
...
describe("dev control-plane management", { timeout: 15_000 }, () => {
  it("backs up and atomically installs verified user units", () => {
    ...
    expect(readFileSync(fixture.env.FAKE_SYSTEMCTL_LOG!, "utf8")).toContain(
      "reset-failed ai-learning-os-api.service ai-learning-os-web.service ai-learning-os-backup.service ai-learning-os-backup-monitor.service ai-learning-os-restore-drill.service",
    );
    expect(readFileSync(fixture.env.FAKE_SYSTEMCTL_LOG!, "utf8")).toContain(
      "start ai-learning-os-backup-monitor.service ai-learning-os-host-capacity-monitor.service",
    );
    expect(readFileSync(fixture.env.FAKE_SYSTEMCTL_LOG!, "utf8")).not.toContain(
      "start ai-learning-os-backup-monitor.service ai-learning-os-application-monitor.service ai-learning-os-host-capacity-monitor.service",
    );
  });
