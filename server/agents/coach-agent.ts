...
import { AGENT_OUTPUT_LIMITS, PublicHttpError, hasOnlyKeys, isBoundedText, isValidAgentTask } from "./request-validation";
...
function validateRequest(request: RecoveryPlanRequest): void {
  if (!hasOnlyKeys(request, ["goal", "currentTask", "interruption"])) throw new PublicHttpError(400, "恢复计划请求格式无效");
  ...
  if (!isValidAgentTask(request.currentTask)) {
    throw new PublicHttpError(400, "当前学习任务格式无效或超出长度限制");
  }
  ...
  if (!hasOnlyKeys(interruption, ["reason", "inactiveDays", "recentDifficultDays", "lastActiveDate"])
    ...
    throw new PublicHttpError(400, "学习中断上下文无效");
  }
  ...
  if ((interruption.reason === "inactivity" && (!hasInactivity || hasRepeatedDifficulty))
    ...
    throw new PublicHttpError(400, "学习中断原因与上下文不一致");
  }
}
