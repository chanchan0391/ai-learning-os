...
function validateRequest(request: TeachingSessionRequest): void {
  if (!hasOnlyKeys(request, ["goal", "task", "learnerContext"])) throw new PublicHttpError(400, "教学请求格式无效");
  ...
  if (!isValidAgentTask(request.task)) throw new PublicHttpError(400, "教学任务格式无效或超出长度限制");
  if (!hasOnlyKeys(request.learnerContext, ["knownConcepts", "recentErrors"])
    ...
    throw new PublicHttpError(400, "学习者上下文格式无效");
  }
}
