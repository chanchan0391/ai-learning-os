...
服务端现在还包含一个不对外暴露的 `InMemorySyncStore` 领域实现，用来固定同步语义并作为未来数据库适配器的可执行契约。调用方必须先从可信会话解析 `userId` 和 `deviceId`，再传入存储层；存储层不会接受客户端提交的所有者字段。当前实现覆盖：
...
它只用于测试领域规则，进程重启会丢失数据，也尚未连接 HTTP API。上线实现必须用事务型数据库适配器替换，并在路由层完成认证、请求校验和限流。
...
2. **进行中：认证与远端仓库。** 已建立 provider-neutral 同步存储契约及用户隔离测试；下一步选择身份方案并实现 PostgreSQL 适配器。
3. **同步试点。** 已验证计划与每日记录的 revision、游标和幂等语义；认证和持久数据库就绪后再接入 HTTP API。
