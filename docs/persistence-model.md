...
这让界面不依赖学习状态的 `localStorage` 键名。`BrowserSyncClient` 另外保存不含学习正文的 revision 和内容指纹，用来判断本地更改、远端更改和两端冲突；`AutoSyncQueue` 只保存是否仍有待同步更改和最近成功时间，不复制学习正文。同步元数据损坏时可以安全丢弃，不影响本地学习记录。当前同步客户端仍只协调所选目标，并会把另一份未归档云端计划视为冲突；多活跃目标的跨设备同步必须先把 revision/指纹元数据改为按计划分区，不能复用单目标选择指针冒充完整支持。
...
6. **进行中：多目标。** 已完成本地多活跃目标集合、旧数据迁移、新建、切换和归档后回退；下一步是按计划隔离同步元数据、批量协调所有活跃目标，再提供跨目标首页。
