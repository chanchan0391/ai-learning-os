...
内存版本只用于测试领域规则，进程重启会丢失数据。PostgreSQL 版本已经通过兼容数据库集成测试，但尚未连接 HTTP API；路由层仍需完成认证、请求校验、CSRF 防护和限流。
...
2. **进行中：认证与远端仓库。** 已选择 OIDC 授权码 + PKCE 与服务端会话方案，并完成 PostgreSQL 适配器、迁移及用户隔离测试；下一步实现会话和认证中间件。
3. **同步试点。** 已验证计划与每日记录的 revision、游标和幂等语义；认证路由就绪后接入 HTTP API。
