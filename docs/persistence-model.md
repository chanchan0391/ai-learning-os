...
进行中目标保存在 `ai-learning-os-active-states-v1` 集合中，集合只记录已完整验证的版本 3 快照和当前选中的计划 ID。原 `ai-learning-os-state-v3` 键继续镜像当前选择，兼容已有导入、同步与升级路径；首次读取旧单目标数据时会自动提升为集合。新建并行目标不会覆盖已有计划，切换只改变选择指针，归档当前目标后会自动选择剩余目标。集合损坏时优先尝试恢复仍可验证的旧单目标镜像。

仓库还用独立的 `ai-learning-os-archived-states-v1` 集合保存已完成目标的完整快照。归档前必须验证计划的全部学习日已经完成；恢复时再次走版本 3 领域校验，并从归档集合移回当前状态键。损坏的当前状态不会连带删除仍可验证的归档。已登录用户归档后，自动同步队列会把完整快照及 `archivedAt` 标记写入原计划实体；同步客户端在寻找活跃目标时忽略带归档标记的云端计划，因此创建下一目标不会产生“另一份活跃计划”冲突。其他设备同步时会完整校验云端归档并合并本地尚不存在的计划 ID，不覆盖已有本地快照，也不会把 `archivedAt` 标记带入可恢复的本地学习状态。

这让界面不依赖学习状态的 `localStorage` 键名。`BrowserSyncClient` 另外保存不含学习正文的 revision 和内容指纹，用来判断本地更改、远端更改和两端冲突；这些元数据按计划 ID 分区，旧单计划元数据会在首次成功写入时升级。同步会依次协调所有本地活跃目标，再下载账号下缺少的未归档目标；仓库批量替换状态时保留当前选择，不会因为后台更新另一目标而切换页面。`AutoSyncQueue` 只保存是否仍有待同步更改和最近成功时间，不复制学习正文。同步元数据损坏时可以安全丢弃，不影响本地学习记录。
...
- 阶段补强来源作为任务产物的可选字段同步，记录来源阶段、学习日、原任务和原始最小行动；掌握度前后变化仍从评估证据实时派生；
...
- 已归档计划在同步表示中额外携带合法 ISO 时间 `archivedAt`；本地归档集合仍把时间与领域快照分开保存，避免让归档状态污染恢复后的活跃计划。
...
- 计划外学习日只允许由最终阶段补强流程创建，必须至少包含一条带阶段、来源学习日和原始最小行动的补强任务；完成后可与全部计划学习日一起导出、同步和归档。
...
6. **已完成同步：多目标。** 已完成本地多活跃目标集合、旧数据迁移、新建、切换、归档后回退、按计划隔离同步元数据、所有活跃目标的双向协调和跨目标首页。
7. **本地节奏偏好。** 跨目标每日总时间预算使用独立版本键保存，取值限制为 15–1440 分钟整数。它不属于任何单一学习计划，也不参与云端 revision；清除全部本地学习数据时一并删除。
