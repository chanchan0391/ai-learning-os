...
4. **结构化输出必须验证。** JSON Schema 约束模型格式、文本长度和集合规模，领域校验独立复核资源边界、标识唯一性、时间预算和学习规则。
...
- 浏览器只保留一个进行中的 Agent 请求；切换目标或卸载页面会中止请求，服务端将客户端断连继续传播到模型提供商，避免旧响应覆盖新证据和无用模型成本
...
实时 Provider 默认要求配置账号预算和 PostgreSQL 会话，否则服务拒绝启动；这避免部署者只加入模型凭据时意外暴露未认证、未计量的付费 Agent API。仅隔离的本地烟雾测试可显式设置 `AI_ALLOW_UNMETERED_LIVE_MODEL=true`，共享和部署环境不得使用。启用账号预算后，五类 Agent 入口要求有效应用会话，并要求浏览器请求的 `Origin` 与部署允许列表精确匹配；跨站请求会在预算查询、读取学习正文和调用 Provider 前拒绝，避免借用已有会话消耗付费模型配额。通过来源检查后，服务再检查当前 UTC 月已入账的 token 与估算金额。成功调用按账号、Agent 动作、Provider、模型、输入 token 和输出 token 写入 PostgreSQL；不保存 Prompt 或模型输出。达到任一上限后返回 `429` 和下月重置时间。价格由部署者按所选模型配置为每百万 token 的美元成本，因此切换模型前必须同步更新费率。

可选的 `AI_GLOBAL_MONTHLY_COST_LIMIT_USD` 使用同一账本汇总所有账号，在应用估算金额达到部署总上限后阻止所有新 Agent 调用，并以不同的安全审计原因区分账号额度和全局额度。订阅强制模式再用 `AI_PLAN_BUDGETS_JSON` 按服务端权益中的套餐键选择账号 token 与金额配额，未配置套餐默认拒绝。该检查由所有实例共享，但仍按已完成调用记账，同一时间已经在途的有限调用可能造成超额；它也不能替代模型厂商控制台中的独立硬上限。生产仍需接入支付 webhook、厂商侧金额上限和集中成本告警。
...
- 组件交互与可访问性：在 `jsdom` 中验证关键数据控制流程并使用 axe-core 扫描渲染后的语义结构；CI 另用固定版本真实 Chromium 扫描首次引导与已填充学习看板，覆盖颜色对比和依赖真实布局的规则，并验证键盘焦点顺序、可见焦点以及破坏性确认对话框的焦点约束、Escape 取消和触发点恢复
