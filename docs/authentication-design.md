...
- 浏览器会话发现、退出、设备清单、单设备撤销、退出所有设备和账号删除统一使用 15 秒请求时限。超时会取消底层请求；会话发现安全降级为仅本地并进入既有有界抖动重试，账号操作则释放忙碌状态、显示固定的隐私安全提示且不清除本地学习记录。状态变更可能已经到达服务端，因此超时界面不会宣称成功；下一次会话或设备读取以服务端当前状态为准。
